...
def parse_dir(dir_path: str | PathLike[str]):
    ...
    # Convert types
    df['map'] = df['map'].str.replace('.map', '', regex=False)
...
def verify_solutions(df):
    inconsistent = df.groupby(['map', 'ID'])['solution'].nunique() > 1
    if inconsistent.any():
...
def verify_fabove(df):
    ...
    problematic_rows = filtered_df[filtered_df['fabove'] != 0]
    if not problematic_rows.empty:
        print("Problematic rows where 'fabove' is not zero:")
        print(problematic_rows)
        raise ValueError(f"'fabove' is not 0 for some cases")


def verify_count(df):
    if df['alg'].value_counts().nunique() != 1:
        raise ValueError(f"'Not all instances were run for all algorithms")


def write_to_excel(df, filename="results/grid.xlsx"):
    with pd.ExcelWriter(filename) as writer:
        df.to_excel(writer, sheet_name="results", index=False)


def main():
    data_dir = r"data/grid"
